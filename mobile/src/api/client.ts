...
export const api = {
  ...
  forgotPassword(email: string) {
    return request<ApiEnvelope<{ accepted: true }>>('/auth/forgot-password', {
      method: 'POST',
      body: JSON.stringify({ email }),
    });
  },
  resetPassword(token: string, password: string) {
    return request<ApiEnvelope<{ success: true }>>('/auth/reset-password', {
      method: 'POST',
      body: JSON.stringify({ token, password }),
    });
  },
  refresh(refreshToken: string) {
    return request<ApiEnvelope<AuthTokensResponse>>('/auth/refresh', {
      method: 'POST',
  ...
  exportAccount() {
    return request<ApiEnvelope<Record<string, unknown>>>('/auth/me/export');
  },
  deleteAccount(password?: string) {
    return request<void>('/auth/me', {
      method: 'DELETE',
      body: JSON.stringify({ confirmation: 'EXCLUIR', ...(password ? { password } : {}) }),
    });
  },
  dashboard() {
